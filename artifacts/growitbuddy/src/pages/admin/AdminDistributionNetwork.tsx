...
export default function AdminDistributionNetwork() {
  ...
  async function handleSave() {
    setSaving(true);
    await saveContent("distribution-network", data);
    setSaving(false);
    setSaved(true);
  }
  ...
  return (
    <div>
      <PageHeader title="Distribution Network Page" desc="Edit all text on the Distribution Network public page." />
      ...
      <Card>
        ...
        <Input label="Section Label" value={data.advantageLabel} onChange={(v) => set("advantageLabel", v)} />
        <Input label="Headline" value={data.advantageHeadline} onChange={(v) => set("advantageHeadline", v)} />
        <Textarea label="Subtext" value={data.advantageSubtext} onChange={(v) => set("advantageSubtext", v)} />
        <div className="mt-4 space-y-3">
          {data.advantageItems.map((item, i) => (
            <div key={i} className="border border-[#0B0B0B]/10 rounded-xl p-4 space-y-2">
              ...
              <Input label="Label" value={item.label} onChange={(v) => setAdvItem(i, { label: v })} />
              <Textarea label="Description" value={item.desc} onChange={(v) => setAdvItem(i, { desc: v })} />
            </div>
          ))}
          <button
            onClick={addAdvItem}
      ...
      <Card>
        ...
        <Input label="Section Label" value={data.hiwLabel} onChange={(v) => set("hiwLabel", v)} />
        <Input label="Headline" value={data.hiwHeadline} onChange={(v) => set("hiwHeadline", v)} />
        <div className="mt-4 space-y-3">
          {data.hiwSteps.map((step, i) => (
            <div key={i} className="border border-[#0B0B0B]/10 rounded-xl p-4 space-y-2">
              ...
              <Input label="Step Number (e.g. 01)" value={step.num} onChange={(v) => setStep(i, { num: v })} />
              <Input label="Title" value={step.title} onChange={(v) => setStep(i, { title: v })} />
              <Textarea label="Description" value={step.desc} onChange={(v) => setStep(i, { desc: v })} />
            </div>
          ))}
          <button
            onClick={addStep}
      ...
      <Card>
        ...
        <Input label="Eyebrow Label" value={data.ctaLabel} onChange={(v) => set("ctaLabel", v)} />
