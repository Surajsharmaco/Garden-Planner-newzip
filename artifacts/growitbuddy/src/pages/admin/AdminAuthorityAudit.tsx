...
export default function AdminAuthorityAudit() {
  ...
  async function handleSave() {
    setSaving(true);
    await saveContent("authority-audit", data);
    setSaving(false);
    setSaved(true);
  }
  ...
  return (
    <div>
      <PageHeader title="Authority Audit Page" desc="Edit all text for the Authority Audit quiz page and its intro cards." />
      ...
      <Card>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="border border-[#0B0B0B]/10 rounded-xl p-4 space-y-2">
            ...
            <Input label="Title" value={data.introFreeTitle} onChange={(v) => set("introFreeTitle", v)} />
            <Textarea label="Description" value={data.introFreeDesc} onChange={(v) => set("introFreeDesc", v)} />
            <Input label="Button Text" value={data.introFreeButton} onChange={(v) => set("introFreeButton", v)} />
          </div>
          <div className="border border-[#0B0B0B]/10 rounded-xl p-4 space-y-2">
            <p className="text-[12px] font-bold text-[#0B0B0B]/40 mb-2">Expert Audit Card</p>
            <Input label="Title" value={data.introPaidTitle} onChange={(v) => set("introPaidTitle", v)} />
            <Textarea label="Description" value={data.introPaidDesc} onChange={(v) => set("introPaidDesc", v)} />
            <Input label="Button Text" value={data.introPaidButton} onChange={(v) => set("introPaidButton", v)} />
          </div>
        </div>
      </Card>
        ...
        <div className="space-y-4">
          {data.questions.map((q, i) => (
            <div key={i} className="border border-[#0B0B0B]/10 rounded-xl p-4 space-y-3">
              ...
              <Input label="ID (unique key, no spaces)" value={q.id} onChange={(v) => setQuestion(i, { id: v })} />
              <Textarea label="Question Text" value={q.question} onChange={(v) => setQuestion(i, { question: v })} />
              {q.type === "text" && (
                <Input label="Placeholder Text" value={q.placeholder ?? ""} onChange={(v) => setQuestion(i, { placeholder: v })} />
              )}
              {q.type === "choice" && (
