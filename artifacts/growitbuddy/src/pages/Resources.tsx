  ...
  return (
    <div className="w-full pt-20 md:pt-24 pb-16 md:pb-24 lg:pb-32 px-5 md:px-8 bg-[#FAFAFA]">
      <div className="container mx-auto max-w-6xl">
        ...
        >
          <h1 className="text-4xl sm:text-5xl md:text-7xl font-bold mb-6 text-[#0A0A0A]">Resource Library.</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
          {resources.map((item, idx) => (
            <motion.div
              ...
              className="bg-[#F4F4F4] border border-black/8 p-6 md:p-8 rounded-3xl shadow-none hover:shadow-md transition-all flex flex-col"
            >
              <div className="text-xs font-bold uppercase tracking-wider text-[#9E9E9E] mb-4">
                {item.type}
              </div>
              <h3 className="text-xl md:text-2xl font-bold mb-4 text-[#0A0A0A]">{item.title}</h3>
              <p className="text-[#525252] text-base mb-8 flex-1">{item.desc}</p>
              ...
              {downloaded[item.title] ? (
                ...
                <Button
                  ...
                  className="w-full h-12 bg-[#0A0A0A] text-white hover:bg-black/85 transition-colors"
                >
      ...
      <Dialog open={!!selectedResource} onOpenChange={(open) => !open && setSelectedResource(null)}>
        <DialogContent className="sm:max-w-md p-6 md:p-8">
          <DialogHeader>
            ...
            <DialogDescription className="text-base text-[#525252]">
          ...
          <form onSubmit={handleDownload} className="mt-6 space-y-4">
            ...
            <Button type="submit" className="w-full h-12 text-base bg-[#0A0A0A] text-black hover:bg-[#0A0A0A]/90 font-bold" disabled={isSubmitting}>
