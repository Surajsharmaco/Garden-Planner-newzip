        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
          {resources.map((item, idx) => (
            <motion.div
              ...
              className="bg-white border border-[#0B0B0B]/[0.08] p-6 md:p-8 rounded-3xl shadow-none hover:shadow-md transition-all flex flex-col"
            >
              <div className="text-xs font-bold uppercase tracking-wider text-[#0B0B0B]/40 mb-4">
                {item.type}
              </div>
              <h3 className="text-xl md:text-2xl font-bold mb-4">{item.title}</h3>
              <p className="text-[#0B0B0B]/50 text-base mb-8 flex-1">{item.desc}</p>
              ...
              {downloaded[item.title] ? (
                ...
                <Button 
                  ...
                  className="w-full h-12 bg-[#F7F7F5] text-[#0B0B0B] hover:bg-accent hover:text-[#0B0B0B] transition-colors"
                >
      ...
      <Dialog open={!!selectedResource} onOpenChange={(open) => !open && setSelectedResource(null)}>
        <DialogContent className="sm:max-w-md p-6 md:p-8">
          <DialogHeader>
            ...
            <DialogDescription className="text-base text-[#0B0B0B]/50">
          ...
          <form onSubmit={handleDownload} className="mt-6 space-y-4">
            ...
            <Button type="submit" className="w-full h-12 text-base bg-accent text-[#0B0B0B] hover:bg-[#FFD84D]/90" disabled={isSubmitting}>
              {isSubmitting ? "Sending..." : "Send it to me"}
            </Button>
            <p className="text-xs text-center text-[#0B0B0B]/40 mt-4">
