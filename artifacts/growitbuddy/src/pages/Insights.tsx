  ...
  return (
    <div className="w-full pt-10 pb-24 bg-white">
      <div className="container mx-auto px-4 md:px-8 max-w-6xl">
        ...
        >
          <h1 className="text-5xl md:text-7xl font-bold mb-6 text-[#0B0B0B]">Insights.</h1>
          <p className="text-xl text-black/45">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {posts.map((post, idx) => (
            ...
            >
              <div className="bg-[#F5F5F5] border border-black/6 aspect-[16/9] rounded-2xl mb-6 overflow-hidden">
                <div className="w-full h-full bg-black/[0.04] transition-transform duration-700 group-hover:scale-105" />
              </div>
              <div className="flex items-center gap-4 mb-3">
                <span className="text-black/50 font-semibold text-sm uppercase tracking-wider">{post.category}</span>
                <span className="text-black/30 text-sm">{post.date}</span>
                <span className="text-black/30 text-sm">• {post.readTime}</span>
              </div>
              <h2 className="text-2xl font-bold mb-3 text-[#0B0B0B] group-hover:text-black/70 transition-colors">{post.title}</h2>
              <p className="text-black/45 line-clamp-2">{post.desc}</p>
            </motion.div>
          ))}
        </div>
      </div>
