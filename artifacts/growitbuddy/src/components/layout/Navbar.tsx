  ...
  return (
    <>
      <header className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-6 md:px-10 pt-5">
        ...
        {/* Logo — all white, no yellow */}
        <Link href="/" className="flex items-center gap-1.5 z-[60]">
          <span className="text-lg font-black tracking-tight text-white">
            GrowitBuddy
          </span>
          <span className="w-1.5 h-1.5 rounded-full bg-white/40 mt-0.5" />
        </Link>
        ...
        <div className="hidden lg:flex items-center gap-2 z-[60]">
          <Link href="/contact">
            <button className="flex items-center gap-2 px-4 py-2 rounded-full border border-white/15 text-sm font-medium text-white/70 hover:text-white hover:border-white/30 hover:bg-white/6 transition-all duration-200 backdrop-blur-sm">
              Get In Touch
              <ArrowUpRight className="w-3.5 h-3.5" />
            </button>
          </Link>
          <Link href="/contact">
            <button className="flex items-center gap-2 px-5 py-2 rounded-full bg-[#F5E663] text-black text-sm font-bold hover:bg-[#F5E663]/90 transition-all duration-200 hover:shadow-[0_0_20px_rgba(245,230,99,0.25)]">
              Book a call
              <ArrowUpRight className="w-3.5 h-3.5" />
            </button>
          </Link>
        ...
        {isOpen && (
          ...
          >
            <nav className="flex flex-col gap-5 flex-1">
              <Link href="/" className={`text-3xl font-black ${location === "/" ? "text-white" : "text-white/60"}`}>
                Home
              </Link>
              {NAV_LINKS.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className={`text-3xl font-black ${location === link.href ? "text-white" : "text-white/60 hover:text-white"}`}
                >
                  {link.label}
                </Link>
              ))}
