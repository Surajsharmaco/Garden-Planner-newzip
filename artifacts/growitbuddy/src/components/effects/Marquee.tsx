          ...
          <span key={idx} style={{ whiteSpace: "nowrap", paddingRight: "2.5rem" }}>
            {items.map((item, i) => (
              <span key={i}>
                ...
                {i < items.length - 1 && (
                  <span style={{ margin: "0 1.2rem", color: dark ? "rgba(255,255,255,0.15)" : "rgba(0,0,0,0.2)", fontSize: "10px" }}>
                    {separator}
                  </span>
                )}
              </span>
            ))}
            <span style={{ marginLeft: "1.2rem", marginRight: "1.2rem", color: dark ? "rgba(255,255,255,0.15)" : "rgba(0,0,0,0.2)", fontSize: "10px" }}>{separator}</span>
          </span>
        ))}
      </div>
    </div>
...
export function ScrollMarquee({ items, dark = false }: { items: string[]; dark?: boolean }) {
  ...
  return (
    <div style={{ overflow: "hidden", background: dark ? "#0A0A0A" : "#F6F6F6", padding: "20px 0" }} ref={containerRef}>
        ...
        {[...items, ...items, ...items].map((item, i) => (
          <span
            ...
            style={{
              ...
              fontWeight: 500,
              color: dark ? (i % 2 === 0 ? "rgba(255,255,255,0.9)" : "rgba(255,255,255,0.15)") : (i % 2 === 0 ? "#000" : "rgba(0,0,0,0.12)"),
              whiteSpace: "nowrap",
